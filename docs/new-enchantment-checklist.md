...
5. Add the enchantment to at least two relevant ordinary affinity tags under `data/betterenchanting/tags/enchantment/`, then add it to exactly one matching `primary/<affinity>` tag. Ordinary memberships all affect essence and modifier pools; only the primary membership affects color and the per-affinity equipment limit.
